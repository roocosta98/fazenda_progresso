    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-lg sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto bg-white">
          ...
          {currentStep === 1 && (
            <div className="divide-y divide-slate-100">
              {filteredLocaisOrigem.length === 0 ? (
                <div className="p-8 text-center text-slate-400 font-medium text-sm">Nenhum local de origem encontrado</div>
              ) : (
          ...
          {currentStep === 2 && (
            <div className="divide-y divide-slate-100">
              {filteredLocaisDestino.length === 0 ? (
                <div className="p-8 text-center text-slate-400 font-medium text-sm">Nenhum local de destino encontrado</div>
              ) : (
          ...
          {currentStep === 4 && (
            <div className="divide-y divide-slate-100">
              ...
              {filteredServicos.length === 0 ? (
                <div className="p-8 text-center text-slate-400 font-medium text-sm">Nenhum tipo de serviço encontrado para esta categoria</div>
              ) : (
          ...
          {currentStep === 5 && (
            <div className="divide-y divide-slate-100">
              {filteredProjetos.length === 0 ? (
                <div className="p-8 text-center text-slate-400 font-medium text-sm">Nenhum projeto encontrado</div>
              ) : (
          ...
          {currentStep === 6 && (
            <div className="divide-y divide-slate-100">
              {filteredSolicitantes.length === 0 ? (
                <div className="p-8 text-center text-slate-400 font-medium text-sm">Nenhum solicitante encontrado</div>
              ) : (
          ...
          {currentStep === 7 && (
            <div className="p-4 bg-slate-50 min-h-full space-y-4 pb-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="pt-2 flex items-center justify-end space-x-3">
                  <button
                    type="button"
                    onClick={handleCloseAll}
                    className="px-4 py-2.5 rounded-xl font-bold text-slate-600 hover:bg-slate-200/60 transition-colors text-sm"
                  >
                    Cancelar
                  </button>
