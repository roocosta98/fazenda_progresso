...
export const NovaSolicitacao: React.FC = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 w-full overflow-x-hidden">
      ...
      <div className="bg-[#1E3A2F] text-white px-4 pt-10 pb-4 sticky top-0 z-30 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center flex-1 mx-2">
            ...
            <div className="text-[11px] font-medium text-[#D4AF37] tracking-wide mt-0.5">
      ...
      {currentStep >= 1 && currentStep <= 6 && (
        <div className="p-4 bg-white border-b border-slate-200 shadow-sm sticky top-[88px] z-20">
          <div className="relative flex items-center bg-slate-50 rounded-2xl border border-slate-200 px-3 py-2.5 focus-within:bg-white focus-within:border-[#D4AF37] focus-within:ring-2 focus-within:ring-[#D4AF37]/20 transition-all">
            <div className="w-8 h-8 rounded-lg bg-slate-100 text-[#1E3A2F] flex items-center justify-center mr-3 shrink-0">
              <Search size={18} />
            </div>
            <input 
              type="text"
      ...
      <div className="flex-1 bg-white">
            ...
            {filteredLocaisOrigem.length === 0 ? (
              ...
              filteredLocaisOrigem.map((loc) => (
                <div 
                  ...
                  className="flex items-center justify-between px-5 py-4 hover:bg-slate-50 cursor-pointer transition-all duration-150 group border-b border-slate-100"
                >
                  <div className="flex items-center space-x-3 pr-4">
                    <MapPin size={18} className="text-[#1E3A2F] shrink-0" />
                    <span className="text-sm font-semibold text-slate-800 uppercase tracking-wide group-hover:text-[#1E3A2F] leading-snug">
                      {loc}
                    </span>
                  </div>
                  <ChevronRight size={18} className="text-slate-400 group-hover:text-[#1E3A2F] group-hover:translate-x-1 transition-all shrink-0" />
                </div>
              ))
            )}
          </div>
            ...
            {filteredCategorias.map((cat) => {
              ...
              return (
                <div 
                  ...
                  className="flex items-center justify-between p-4 rounded-2xl border border-slate-200 hover:border-[#D4AF37] hover:bg-slate-50 cursor-pointer transition-all shadow-sm group bg-white"
                >
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 rounded-xl bg-[#D4AF37]/20 text-[#1E3A2F] flex items-center justify-center shrink-0 group-hover:bg-[#1E3A2F] group-hover:text-white transition-colors">
                      <IconeCat size={22} />
                    </div>
                    <div>
                      <h4 className="text-base font-bold text-slate-800 group-hover:text-[#1E3A2F]">
                        {cat.nome}
                      </h4>
                  ...
                  <ChevronRight size={20} className="text-slate-400 group-hover:text-[#1E3A2F] group-hover:translate-x-1 transition-all shrink-0" />
                </div>
              );
            })}
          </div>
        ...
        {currentStep === 4 && (
          <div className="divide-y divide-slate-100">
            <div className="bg-slate-100 px-5 py-2.5 text-xs font-bold text-[#1E3A2F] uppercase tracking-wide border-b border-slate-200">
            ...
            {filteredServicos.length === 0 ? (
              ...
              filteredServicos.map((s) => (
                ...
                >
                  <div className="flex items-center space-x-3 pr-4">
                    <Wrench size={18} className="text-slate-400 group-hover:text-[#1E3A2F] shrink-0" />
                    <span className="text-sm font-semibold text-slate-800 uppercase tracking-wide group-hover:text-[#1E3A2F] leading-snug">
                      {s.descricao}
                    </span>
                  </div>
          ...
          <div className="divide-y divide-slate-100">
            {filteredProjetos.length === 0 ? (
              ...
              filteredProjetos.map((p) => (
                ...
                >
                  <div className="flex items-center space-x-3 pr-4">
                    <FolderKanban size={18} className="text-slate-400 group-hover:text-[#1E3A2F] shrink-0" />
                    <span className="text-sm font-semibold text-slate-800 uppercase tracking-wide group-hover:text-[#1E3A2F] leading-snug">
                      {p.descricao}
                    </span>
                  </div>
              ...
              filteredSolicitantes.map((sol) => {
                ...
                return (
                  <div 
                    ...
                    className={`flex items-center justify-between px-5 py-4 hover:bg-slate-50 cursor-pointer transition-all duration-150 group border-b border-slate-100 ${
                      isEu ? 'bg-slate-100/60 font-bold' : ''
                    }`}
                  >
                    <div className="flex items-center space-x-3 pr-4">
                      <User size={18} className={`${isEu ? 'text-[#1E3A2F] font-bold' : 'text-slate-400'} group-hover:text-[#1E3A2F] shrink-0`} />
                      <span className={`text-sm tracking-wide uppercase group-hover:text-[#1E3A2F] leading-snug ${
          ...
          <div className="p-5 bg-slate-50 min-h-full space-y-5 pb-12">
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-3">
              <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                ...
                <span className="text-[11px] text-[#1E3A2F] font-bold">6 de 6 Definidos</span>
              </div>

              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="font-semibold text-slate-600 flex items-center"><MapPin size={14} className="mr-2 text-[#1E3A2F]" /> Origem:</span>
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-slate-800 truncate max-w-[170px]">{origemSel}</span>
                    <button onClick={() => setCurrentStep(1)} className="text-[#1E3A2F] hover:text-[#1E3A2F] p-1" title="Alterar">
                      <Edit3 size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button onClick={() => setCurrentStep(2)} className="text-[#1E3A2F] hover:text-[#1E3A2F] p-1" title="Alterar">
                      <Edit3 size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="font-semibold text-slate-600 flex items-center"><Wrench size={14} className="mr-2 text-[#1E3A2F]" /> Serviço:</span>
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-slate-800 truncate max-w-[170px]">{servicoSel?.descricao}</span>
                    <button onClick={() => setCurrentStep(3)} className="text-[#1E3A2F] hover:text-[#1E3A2F] p-1" title="Alterar">
                      <Edit3 size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="font-semibold text-slate-600 flex items-center"><FolderKanban size={14} className="mr-2 text-[#1E3A2F]" /> Projeto:</span>
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-slate-800 truncate max-w-[170px]">{projetoSel?.descricao}</span>
                    <button onClick={() => setCurrentStep(5)} className="text-[#1E3A2F] hover:text-[#1E3A2F] p-1" title="Alterar">
                      <Edit3 size={14} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-2 rounded-xl bg-slate-50 border border-slate-100">
                  <span className="font-semibold text-slate-600 flex items-center"><User size={14} className="mr-2 text-[#1E3A2F]" /> Solicitante:</span>
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-slate-800 truncate max-w-[170px]">{solicitanteSel?.descricao}</span>
                    <button onClick={() => setCurrentStep(6)} className="text-[#1E3A2F] hover:text-[#1E3A2F] p-1" title="Alterar">
                      <Edit3 size={14} />
                    </button>
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-3">
                <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center border-b border-slate-100 pb-2">
                  <Calendar size={16} className="mr-2 text-[#1E3A2F]" /> Programação
                </h3>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input 
                      type="date" 
                      required
                      className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-800 shadow-sm focus:ring-2 focus:ring-[#D4AF37]"
              ...
              <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm space-y-2">
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide flex items-center justify-between">
                  <span className="flex items-center">
                    <FileText size={16} className="mr-2 text-[#1E3A2F]" /> Observações Adicionais <span className="text-red-500 ml-1">*</span>
                  </span>
                ...
                <textarea
                  ...
                  className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-[#D4AF37] resize-none shadow-sm font-medium"
                  value={observacoes}
              ...
              <div className="pt-3 flex items-center justify-end space-x-3">
                ...
                <button
                  ...
                  className="bg-[#1E3A2F] hover:bg-[#2D5A46] text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-[#1E3A2F]/20 flex items-center text-sm disabled:opacity-50"
                >
                  {isLoading ? (
      ...
      {successModalOpen && (
        <div className="fixed inset-0 z-60 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="w-16 h-16 bg-[#D4AF37]/20 text-[#1E3A2F] rounded-full flex items-center justify-center mx-auto mb-4 relative">
            ...
            <button
              ...
              className="w-full bg-[#1E3A2F] text-white py-3 rounded-xl font-bold hover:bg-[#2D5A46] transition-colors shadow-lg text-sm flex items-center justify-center"
            >
              Ver Minhas Viagens
            </button>
          </div>
