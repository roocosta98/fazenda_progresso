...
export const Sidebar = ({ mobileAberto = false, onFechar }: SidebarProps) => {
  ...
  const itensPorModuloBase: Record<ModuloSistema, (LinkItem | GrupoItem)[]> = {
    ...
    estoque: [
      ...
      { to: '/logistica/estoque/central-de-acoes', icon: <ListChecks size={18} />, label: 'Central de Ações' },
      { to: '/logistica/estoque/inventario', icon: <List size={18} />, label: 'Inventário' },
      { to: '/logistica/estoque/cotacoes', icon: <ClipboardCheck size={18} />, label: 'Compras & Cotações' },
      { to: '/logistica/estoque/fornecedores', icon: <Handshake size={18} />, label: 'Fornecedores' },
      { to: '/logistica/estoque/analises', icon: <LineChart size={18} />, label: 'Análises' },
