...
function montarConsultas(dataInicio: string, dataFim: string, dias: number) {
  ...
  // Itens de cotação cancelados não representam disputa real entre fornecedores — contá-los
  // como participação/derrota distorce a taxa de vitória, por isso ficam fora do ranking.
  fornecedores: `SELECT TOP 2000 PAR.NOMEPARC AS FORNECEDOR,
    ...
    WHERE ITC.CODPARC <> 0 AND UPPER(LTRIM(RTRIM(CAST(ITC.SITUACAO AS VARCHAR(20))))) NOT IN ('C','CANCELADA') AND ${filtroGrupo}
    GROUP BY PAR.NOMEPARC
    ORDER BY TOTALVENCIDAS DESC, TOTALCOTACOES DESC`,
  // Um item de cotação só está de fato "em aberto" se ainda não gerou nota de compra
  // (NUNOTACPA vazio) E ainda não estiver Fechado/Cancelado (TGFITC.SITUACAO) — as duas
  // condições precisam valer ao mesmo tempo; se qualquer uma indicar conclusão, não é mais aberto.
  cotacoes: `SELECT TOP 2000 COT.NUMCOTACAO, COT.DHINIC, COT.DHFINAL, USU.NOMEUSU AS COMPRADOR,
      (SELECT COUNT(*) FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND ${filtroGrupo}) AS TOTALITENS,
      (SELECT COUNT(*) FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.NUNOTACPA IS NULL AND UPPER(LTRIM(RTRIM(CAST(I.SITUACAO AS VARCHAR(20))))) NOT IN ('F','C','FECHADA','CANCELADA') AND ${filtroGrupo}) AS ITENSEMABERTO
    FROM TGFCOT COT LEFT JOIN TSIUSU USU ON USU.CODUSU=COT.CODUSUREQ
    WHERE EXISTS (SELECT 1 FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.NUNOTACPA IS NULL AND UPPER(LTRIM(RTRIM(CAST(I.SITUACAO AS VARCHAR(20))))) NOT IN ('F','C','FECHADA','CANCELADA') AND ${filtroGrupo})
  ...
  kpis: `SELECT
    ...
    (SELECT COUNT(DISTINCT COT.NUMCOTACAO) FROM TGFCOT COT WHERE EXISTS (SELECT 1 FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.NUNOTACPA IS NULL AND UPPER(LTRIM(RTRIM(CAST(I.SITUACAO AS VARCHAR(20))))) NOT IN ('F','C','FECHADA','CANCELADA') AND ${filtroGrupo})) AS TOTALCOTACOES,
