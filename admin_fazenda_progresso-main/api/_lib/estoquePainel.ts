...
function montarConsultas(dataInicio: string, dataFim: string, dias: number) {
  ...
  semMovimentacao: `SELECT
    ...
    FROM (
      ...
      WHERE E.CODEMP = 1
        AND G.CODGRUPAI NOT IN (9000000,13000000,15000000,16000000,22000000,23000000,24000000,25000000,27000000,29000000,98000000)
  ...
  fornecedores: `SELECT TOP 2000 PAR.NOMEPARC AS FORNECEDOR,
    ...
    WHERE ITC.CODPARC <> 0 AND UPPER(LTRIM(RTRIM(CAST(ITC.SITUACAO AS VARCHAR(20))))) NOT IN ('C','CANCELADA')
  ...
  cotacoes: `SELECT TOP 2000 COT.NUMCOTACAO, COT.DHINIC, COT.DHFINAL, USU.NOMEUSU AS COMPRADOR,
      ...
      (SELECT COUNT(*) FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.NUNOTACPA IS NULL AND UPPER(LTRIM(RTRIM(CAST(I.SITUACAO AS VARCHAR(20))))) NOT IN ('F','C','FECHADA','CANCELADA')) AS ITENSEMABERTO
    FROM TGFCOT COT LEFT JOIN TSIUSU USU ON USU.CODUSU=COT.CODUSUREQ
    WHERE EXISTS (SELECT 1 FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.NUNOTACPA IS NULL AND UPPER(LTRIM(RTRIM(CAST(I.SITUACAO AS VARCHAR(20))))) NOT IN ('F','C','FECHADA','CANCELADA'))
  ...
  // O filtro de grupo (CODGRUPAI NOT IN (...)) fica só nos KPIs abaixo (usados no Dashboard/cards
  // gerais de estoque) — giro por produto (detalhe, acima) e as demais telas mostram todos os grupos.
  kpis: `SELECT
    ...
    (SELECT COUNT(DISTINCT COT.NUMCOTACAO) FROM TGFCOT COT WHERE EXISTS (SELECT 1 FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.NUNOTACPA IS NULL AND UPPER(LTRIM(RTRIM(CAST(I.SITUACAO AS VARCHAR(20))))) NOT IN ('F','C','FECHADA','CANCELADA'))) AS TOTALCOTACOES,
