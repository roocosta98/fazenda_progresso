...
function montarConsultas(dataInicio: string, dataFim: string, dias: number) {
  ...
  // Itens de cotação cancelados (STATUSPRODCOT='C') não representam disputa real entre
  // fornecedores — contá-los como participação/derrota distorce a taxa de vitória, por
  // isso ficam fora do ranking.
  fornecedores: `SELECT TOP 2000 PAR.NOMEPARC AS FORNECEDOR,
    ...
    WHERE ITC.CODPARC <> 0 AND ITC.STATUSPRODCOT <> 'C' AND ${filtroGrupo}
  ...
  // C=Cancelada, E=Enviada, F=Fechada, P=Precificada) — é o mesmo campo que aparece na tela
  // "Situação do produto" do Sankhya e o único confiável pra saber se um item ainda está em
  // aberto. TGFITC.SITUACAO é outro campo (usado noutro contexto, ligado a envio/coleta de
  // preço) e NUNOTACPA sozinho não bastava — confirmado com o Eder (grupo do WhatsApp) comparando
  // a contagem por STATUSPRODCOT direto no Sankhya com o painel.
  cotacoes: `SELECT TOP 2000 COT.NUMCOTACAO, COT.DHINIC, COT.DHFINAL, USU.NOMEUSU AS COMPRADOR,
      ...
      (SELECT COUNT(*) FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.STATUSPRODCOT='O' AND ${filtroGrupo}) AS ITENSEMABERTO
    FROM TGFCOT COT LEFT JOIN TSIUSU USU ON USU.CODUSU=COT.CODUSUREQ
    WHERE EXISTS (SELECT 1 FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.STATUSPRODCOT='O' AND ${filtroGrupo})
  ...
  kpis: `SELECT
    ...
    (SELECT COUNT(DISTINCT COT.NUMCOTACAO) FROM TGFCOT COT WHERE EXISTS (SELECT 1 FROM TGFITC I INNER JOIN TGFPRO PRO ON PRO.CODPROD=I.CODPROD INNER JOIN TGFGRU GRU ON GRU.CODGRUPOPROD=PRO.CODGRUPOPROD WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.STATUSPRODCOT='O' AND ${filtroGrupo})) AS TOTALCOTACOES,
