...
function montarConsultas(dataInicio: string, dataFim: string, dias: number) {
  ...
  semMovimentacao: `SELECT TOP 100 P.CODPROD, P.DESCRPROD, L.DESCRLOCAL AS LOCAL, G.GIRODIARIO, G.DIASSEMVENDA, G.ESTCUSTGER AS VALORESTOQUE
  ...
  cotacoes: `SELECT TOP 100 COT.NUMCOTACAO, COT.DHINIC, COT.DHFINAL, USU.NOMEUSU AS COMPRADOR,
      (SELECT COUNT(*) FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO) AS TOTALITENS,
      (SELECT COUNT(*) FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND CAST(I.SITUACAO AS VARCHAR(20)) NOT IN ('F','C')) AS ITENSEMABERTO
  ...
  giroProdutos: `WITH MOVIMENTOS AS (
    ...
    SELECT TOP 100 MOV.CODPROD, PRO.DESCRPROD,
  ...
  kpis: `SELECT
    ...
    (SELECT COUNT(DISTINCT COT.NUMCOTACAO) FROM TGFCOT COT WHERE EXISTS (SELECT 1 FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND CAST(I.SITUACAO AS VARCHAR(20)) NOT IN ('F','C'))) AS TOTALCOTACOES,
