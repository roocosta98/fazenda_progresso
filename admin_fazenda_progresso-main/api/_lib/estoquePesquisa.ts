...
const TABELAS_PERMITIDAS = new Set(['TGFPRO', 'TGFEST', 'TGFLOC', 'TGFGIR', 'TGFCUS', 'TGFITE', 'TGFCAB', 'TGFTOP', 'TGFPAR', 'TGFITC', 'TGFCOT', 'TSIUSU', 'TGFGRU']);
// Grupos de produto sem relevância operacional pro controle de estoque (materiais de escritório,
// imobilizado, serviços, energia, matéria-prima de beneficiamento, produtos obsoletos etc) —
// mesma lista usada no painel estruturado (estoquePainel.ts). Um caso real: "item mais requisitado"
// trouxe "BATATA SUJA" (matéria-prima do beneficiamento, grupo 29000000), que não é item de
// almoxarifado de verdade — confirmado com o Marcos.
const GRUPOS_EXCLUIDOS = '9000000,13000000,15000000,16000000,22000000,23000000,24000000,25000000,27000000,29000000,98000000';
const ESQUEMA = `TGFPRO(CODPROD,DESCRPROD,REFERENCIA,MARCA,ATIVO,ESTMIN,ESTMAX,CODGRUPOPROD); TGFEST(CODPROD,CODLOCAL,CODEMP,CONTROLE,ESTOQUE); TGFGIR(CODPROD,CODLOCAL,CODEMP,GIRODIARIO,DIASSEMVENDA,ESTCUSTGER,PRODFALTA,ESTMINGIR,PONTOPED); TGFCUS(CODPROD,CODEMP,CUSMEDICM,CUSSEMICM,DTATUAL,NUNOTA); TGFLOC(CODLOCAL,DESCRLOCAL); TGFITE(NUNOTA,CODPROD,QTDNEG,CUSTO); TGFCAB(NUNOTA,CODEMP,DTNEG,CODTIPOPER,CODPARC); TGFTOP(CODTIPOPER,ATUALEST,DESCROPER); TGFPAR(CODPARC,NOMEPARC); TGFITC(NUMCOTACAO,CODPROD,CODPARC,CODLOCAL,CONTROLE,DIFERENCIADOR,CABECALHO,SITUACAO,STATUSPRODCOT,NUNOTACPA,CONFIABFORN,QUALATEND,QUALPROD,PRAZOENTREGA,MELHOR); TGFCOT(NUMCOTACAO,DHINIC,DHFINAL,SITUACAO,CODUSUREQ); TSIUSU(CODUSU,NOMEUSU); TGFGRU(CODGRUPOPROD,CODGRUPAI).`;
...
function consultaAtalho(pergunta: string) {
  ...
  if (/cota[cç][aã]o|cota[cç][oõ]es/.test(texto) && /em\s+abert/.test(texto)) return `SELECT TOP 100 COT.NUMCOTACAO, COT.DHINIC, COT.DHFINAL, USU.NOMEUSU AS COMPRADOR
    ...
    WHERE EXISTS (SELECT 1 FROM TGFITC I WHERE I.NUMCOTACAO=COT.NUMCOTACAO AND I.CABECALHO='S' AND I.STATUSPRODCOT='O')
...
export async function pesquisarEstoque(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const promptBase = `Você é um assistente de estoque Sankhya. Converta perguntas em SQL SOMENTE LEITURA. Use exclusivamente este schema: ${ESQUEMA} Responda JSON {"sql":"..."}. Use SELECT ou WITH, no máximo TOP 100; nunca use ponto-e-vírgula, DML, metadados ou tabelas fora da lista. Use apenas as colunas exatamente como aparecem entre parênteses de cada tabela acima — nunca misture uma coluna de uma tabela com outra tabela, mesmo que os nomes pareçam relacionados. REGRA DE NOME DE TABELA: os únicos nomes de tabela que existem são exatamente estes: ${[...TABELAS_PERMITIDAS].join(', ')}. Nunca invente, abrevie ou parafraseie um nome de tabela (ex.: "EST" ou "ESTOQUE" não são tabelas — a tabela certa é TGFEST, e ESTOQUE é uma coluna dela, não uma tabela). Se usar CTE (WITH nome AS (...)), o nome da CTE não é uma tabela real, mas toda tabela referenciada dentro do FROM/JOIN da CTE e do restante da consulta precisa ser um desses nomes exatos. REGRA DE EMPRESA: o sistema opera apenas com a empresa 1 (CODEMP = 1); sempre filtre CODEMP = 1 em TGFEST, TGFGIR, TGFCUS e TGFCAB, mesmo que a pergunta não mencione empresa. REGRA DE COTAÇÃO: nem TGFCOT.SITUACAO nem TGFITC.SITUACAO indicam se um item de cotação está de fato em aberto (SITUACAO é outro campo, ligado a envio/coleta de preço) — o campo oficial é TGFITC.STATUSPRODCOT (a "Situação do produto" mostrada no próprio Sankhya), com os códigos: O=Aberta, A=Aprovada, C=Cancelada, E=Enviada, F=Fechada, P=Precificada. TGFITC tem UMA linha resumo por produto (CABECALHO='S', CODPARC=0) com o STATUSPRODCOT confiável, e uma linha por fornecedor cotado (CABECALHO='N', CODPARC<>0) cujo STATUSPRODCOT fica desatualizado; por isso, pra saber se um item/cotação está aberto, cancelado ou fechado, filtre SEMPRE CABECALHO='S' (nunca decida isso pela linha do fornecedor). Um item só está em aberto se, na linha resumo (CABECALHO='S'), STATUSPRODCOT='O'. Pra listar por fornecedor (preço, prazo, quem cotou), use as linhas CABECALHO='N' AND CODPARC<>0, mas cruze com a linha resumo (mesmo NUMCOTACAO+CODPROD+CODLOCAL+CONTROLE+DIFERENCIADOR) se precisar saber a situação real. Nunca use SITUACAO nem NUNOTACPA pra decidir se algo está aberto/fechado/cancelado — use sempre STATUSPRODCOT da linha resumo. REGRA DE GRUPO DE PRODUTO: em perguntas sobre consumo, requisição, item mais/menos movimentado, giro ou "itens do estoque" em geral, exclua produtos dos grupos irrelevantes pro almoxarifado operacional (junte TGFPRO com TGFGRU pelo CODGRUPOPROD e filtre GRU.CODGRUPAI NOT IN (${GRUPOS_EXCLUIDOS}) — inclui matéria-prima de beneficiamento, materiais de escritório, imobilizado, serviços, energia elétrica e produtos obsoletos), a menos que a pergunta peça especificamente por esses itens. REGRA DE NEGÓCIO — NÃO É VAREJO: esta fazenda (produção de batata, uva/vinícola e café) não vende ao consumidor final pelo estoque; toda saída de TGFGIR/TGFEST é consumo interno — insumo pra produção agrícola, manutenção de veículos/equipamentos, ou baixa —, nunca "venda". TGFGIR.DIASSEMVENDA é um campo herdado do Sankhya (nome de fábrica do ERP, pensado pra varejo) mas aqui significa "dias sem saída de estoque"; ao responder, NUNCA use a palavra "venda" — diga "saída", "consumo" ou "movimentação".${contextoTreinamento}`;
